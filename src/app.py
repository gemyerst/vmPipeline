...
def handle_single_request(request: PipelineRequest, gcs_client: StorageClient, experiment_dir: Path, visionnerf_script: Optional[Path], nvdiffrec_script: Optional[Path]) -> None:
    ...
    if visionnerf_script is not None:
        ...
        LOGGER.info(f"{request.runId} | VisionNerf | Skipping script run.")
    ...
    if nvdiffrec_script is not None:
        ...
        LOGGER.info(f"{request.runId} | Nvdiffrec | Skipping script run.")
        ...
        try:
            while True:
                ...
                try:
                    ...
                    run_bash_command(log_prefix="GSUtil Visionnerf", args=["gsutil", "-m", "cp", "-R", str(visionnerf_results_dir.resolve()), f"gs://{request.outputDataBucket}/{request.runId}/visionnerf"])
                    ...
                    run_bash_command(log_prefix="GSUtil Nvdiffrec", args=["gsutil", "-m", "cp", "-R", str(nvdiffrec_results_dir.resolve()), f"gs://{request.outputDataBucket}/{request.runId}/nvdiffrec"])
